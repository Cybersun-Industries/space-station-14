...
public sealed partial class SurgerySystem
{
    ...
    private void OnAddSurgeryEventPostAction(AddSurgeryEvent ev)
    {
        if (!TryGetOperationPrototype(ev.PrototypeId, out var operationPrototype))
            return;
        if (!TryComp<Common.Medical.Surgery.Components.SurgeryInProgressComponent>(ev.Uid, out var surgeryInProgressComponent) ||
            surgeryInProgressComponent.CurrentStep == null)
            return;


        var step = Enum.Parse<SurgeryTypeEnum>(surgeryInProgressComponent.CurrentStep.Key.ToString());
        var partType = Enum.Parse<BodyPartType>(operationPrototype.BodyPart);
        if (step == SurgeryTypeEnum.AddPart)
        {
            ...
            var slotId = _bodySystem.GetBodyAllSlots(ev.Uid).First(g => g.Type == partType).Id;
            slotId = Enum.Parse<BodyPartSymmetry>(surgeryInProgressComponent.Symmetry.ToString()) ==
                      BodyPartSymmetry.Left
                ? slotId.Replace("right", "left")
                : slotId.Replace("left", "right");
            _bodySystem.AttachPart(root.Value.Entity, slotId, ev.PartUid);
            return;
        }
    ...
    private void OnBurnSurgeryPostAction(BurnSurgeryEvent ev)
    {
        ...
        if (damagedParts.ToList().Count == 0)
        {
            ...
            RemComp<Common.Medical.Surgery.Components.SurgeryInProgressComponent>(ev.Uid);
            return;
        }
        ...
        foreach (var damagedPart in damagedParts)
        {
            ...
            if (part.ToList().Count != 0)
            {
                ...
                _popupSystem.PopupEntity("Рана обработана.", ev.Uid, PopupType.Medium);
            }
    ...
    private void OnLRepairCompFSurgeryPostAction(RepairCompFSurgeryEvent ev)
    {
        ...
        foreach (var damagedPart in damagedParts)
        {
            ...
            if (part.ToList().Count != 0)
            {
                ...
                _popupSystem.PopupEntity("Кость восстановлена.", ev.Uid, PopupType.Medium);
            }
    ...
    private void OnRepairBoneFSurgeryPostAction(RepairBoneFSurgeryEvent ev)
    {
        ...
        foreach (var damagedPart in damagedParts)
        {
            ...
            {
                damagedPart.Component.Wounds.RemoveAll(i => i.Type == WoundTypeEnum.Blunt);
                _popupSystem.PopupEntity("Кость зафиксирована.", ev.Uid, PopupType.Medium);
            }

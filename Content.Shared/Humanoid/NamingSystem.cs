    ...
    public sealed class NamingSystem : EntitySystem
    {
            ...
            switch (speciesProto.Naming)
            {
                ...
                case SpeciesNaming.FirstDashFirst:
                    return Loc.GetString("namepreset-firstdashfirst",
                        ...
                        ("first2", GetFirstName(speciesProto, gender))); // Corvax-LastnameGender
        ...
        public string GetLastName(SpeciesPrototype speciesProto, Gender? gender = null)
        {
            switch (gender)
            {
                ...
                case Gender.Female:
                    return _random.Pick(
                        _prototypeManager.Index<LocalizedDatasetPrototype>(speciesProto.FemaleLastNames));
                default:
                    if (_random.Prob(0.5f))
                        return _random.Pick(
                            _prototypeManager.Index<LocalizedDatasetPrototype>(speciesProto.MaleLastNames));
                    else
                        return _random.Pick(
                            _prototypeManager.Index<LocalizedDatasetPrototype>(speciesProto.FemaleLastNames));
            }

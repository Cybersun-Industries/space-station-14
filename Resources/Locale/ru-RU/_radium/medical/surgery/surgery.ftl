surgery-organOperation-name = Манипулирование органами
surgery-organOperation-desc = Эта операция охватывает операции по удалению/вставке органов.

surgery-featureOperation-name = Манипулирование внешними органами
surgery-featureOperation-desc = Эта операция охватывает операции по удалению/вставке внешних органов. Например: Уши, крылья, волосы...

surgery-LobectomyOperation-name = Лобэктомия
surgery-LobectomyOperation-desc = Эта операция позволяет полностью вылечить лёгкие, но может быть проведена только один раз. Не путать с лоботомией!

surgery-coronaryOperation-name = Коронарное шунтирование
surgery-coronaryOperation-desc = Эта операция позволяет полностью вылечить и запустить сердце, но может быть проведена только один раз.

surgery-hepatectomyOperation-name = Гепатэктомия
surgery-hepatectomyOperation-desc = Удаляет поврежденные части печени, чтобы печень могла должным образом регенерировать. Операция доступна только если печень не отказала. Может быть проведена только один раз.

surgery-gastrectomyOperation-name = Гастрэктомия
surgery-gastrectomyOperation-desc = Удаляет часть двенадцатиперстной кишки, прикрепляя желудок обратно к нижним отделам кишечника.  Позволяет полностью вылечить желудок, но может быть проведена только один раз.

surgery-amputationOperation-name = Ампутация
surgery-amputationOperation-desc = Эта хирургическая операция позволяет ампутировать конечность.

surgery-implantionOperation-name = Имплантация
surgery-implantionOperation-desc = Эта хирургическая операция позволяет имплантировать что-либо.

surgery-implantRemovalOperation-name = Удаление импланта
surgery-implantRemovalOperation-desc = Эта хирургическая операция позволяет удалить имплант.

surgery-brainOperation-name = Операция на головном мозге
surgery-brainOperation-desc = Эта процедура излечивает все тяжелые и элементарные травмы и значительно уменьшает повреждение головного мозга.

surgery-eyeOperation-name = Коррекция зрения
surgery-eyeOperation-desc = Эта процедура устраняет все повреждения, нанесенные глазам и устраняет слепоту.

surgery-eyeBlindOperation-name = Ослепление
surgery-eyeBlindOperation-desc = Эта процедура перманентно ослепляет пациента.

surgery-revivalOperation-name = Продвинутая реанимация
surgery-revivalOperation-desc = Реанимирует любой труп. Если труп слишком сильно повреждён то пациент умрёт сразу же.

surgery-filterOperation-name = Фильтрация крови
surgery-filterOperation-desc = Эта операция позволяет удалить из крови все реагенты.

surgery-pumpOperation-name = Промывание желудка
surgery-pumpOperation-desc = Вызывает у пациента рвоту, которая удаляет большую часть проглоченных реагентов из желудка.

surgery-reapirBFOperation-name = Восстановление перелома кости
surgery-reapirBFOperation-desc = Эта операция восстанавливает трещины в костях.

surgery-reapirCompBFOperation-name = Восстановление составного перелома кости
surgery-reapirCompBFOperation-desc = Эта операция восстанавливает переломанные кости.

surgery-burnOperation-name = Обработка инфицированной плоти
surgery-burnOperation-desc = Эта операция удаляет ожоги.

surgery-pierceOperation-name = Шунтирование прокола.
surgery-pierceOperation-desc = Эта операция удаляет проколы.

surgery-addOperationL-name = Пришить ногу
surgery-addOperationL-desc = Эта операция позволяет вернуть утраченную конечность на место.

surgery-addOperationA-name = Пришить руку
surgery-addOperationA-desc = Эта операция позволяет вернуть утраченную конечность на место.

surgery-scalpelAction-name = Скальпель
surgery-incisionAction-desc = Сделайте надрез.
surgery-exciseLungAction-desc = Вырежьте повреждённую часть лёгкого.
surgery-inciseHeartAction-desc = Надрежьте сердце.
surgery-exciseLiverAction-desc = Вырежьте повреждённую часть печени.
surgery-removeDuodenumAction-desc = Удалите нижнюю часть двенадцатиперстной кишки.
surgery-exciseInfectionAction-desc = Удалите инфекцию.

surgery-retractorAction-name = Ретрактор
surgery-retractAction-desc = Раздвиньте кожу.

surgery-sawAction-name = Циркулярная пила
surgery-sawAction-desc = Распилите кость.
surgery-severLimbAction-desc = Отрубите конечность.

surgery-hemostatAction-name = Гемостат
surgery-clampAction-desc = Зажмите сосуды.
surgery-removeAction-desc = Удалите что-либо.
surgery-attachHeartAction-desc = Прикрепите сердечный трансплантат.
surgery-cauterizeStomachAction-desc = Восстановите нижнюю часть живота.
surgery-extractImplantAction-desc = Вытащите имплант.
surgery-brainAction-desc = Проведите операцию на мозге.
surgery-eyeAction-desc = Вылечите глаз.
surgery-eyeAction-name = Откорректируйте зрение.
surgery-eyeActionB-desc = Повредите глаз.
surgery-bypassHemoAction-desc = Соедените сосуды.

surgery-handsAction-name = Руки
surgery-handsInsAction-desc = Вставьте что-либо.
surgery-cavityAction-desc = Вставьте или достаньте что-либо из полости
surgery-pumpAction-desc = Промойте желудок.

surgery-CauteryAction-name = Прижигатель
surgery-sealAction-desc = Прижгите разрез.
surgery-mendAction-desc = Закройте надрез.
surgery-bypassCautAction-desc = Зашунтируйте сосуды.

surgery-reviveAction-name = Дефибриллятор
surgery-reviveAction-desc = Реанимируйте пациента

surgery-filterAction-name = Фильтр крови
surgery-filterAction-desc = Профильтруйте кровь.

surgery-BBgStAction-name = Корректирующая сеть
surgery-BgStAction-name = Корректирующая сеть
surgery-reapirAction-desc = Восстановите кость.

surgery-bonesetterAction-name = Корректирующая сеть
surgery-resetBoneAction-desc = Верните кость в исходное положение.

surgery-MgStAction-name = Бинты
surgery-bandageAction-desc = Перевяжите ожоги.

surgery-addFirstPartL-name = Нога
surgery-addFirstPartL-desc = Пришейте ногу.

surgery-addSecondPartL-name = Ступня
surgery-addSecondPartL-desc = Пришейте ступню.

surgery-addFirstPartA-name = Рука
surgery-addFirstPartA-desc = Пришейте руку.

surgery-addSecondPartA-name = Кисть
surgery-addSecondPartA-desc = Пришейте кисть.

health-analyzer-window-currentOperation = Текущая операция:
health-analyzer-window-instructions = Указания к операции:
health-analyzer-window-noOperation = Операция не проводится.

surgery-window-title = Начать операцию

surgery-target-shouldBuckled = Цель должна быть пристёгнута.
surgery-target-begin = Начинаю операцию.

surgery-bleeding = Вы теряете кровь!
